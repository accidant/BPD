...
safe_query("INSERT INTO ".PREFIX."topmatch (date, matchlink, league, maps, server, teamclan, country1, team1, homepage1, country2, team2, homepage2, displayed, game)
	             values('$date', '$matchlink', '$league', '$maps', '$server','$teamclan','$country1', '$team1', '$homepage1', '$country2', '$team2', '$homepage2', '$displayed', '$game' )");
...
if($logo1['name'] != "") {
			...
			if($getimg[0] < $logowidth+1 && $getimg[1] < $logoheight+1) {
				$pic = '';
...
function hidd(id)
{
    if(document.getElementById(id).style.display==\"none\")
		...
		echo"<script language=\"JavaScript\" type=\"text/javascript\">
			function chkFormular() {
				...
				var timestamp_set=Date.UTC(year,month,day); // timestamp in ms vom Datum
				var timestamp_now=Date.UTC(jjjj,mm,tt); // timestamp von heute
				
				if (timestamp_set<timestamp_now) {
					alert('Das Datum befindet sich in der Vergangenheit - The date is in the past');
					return false;
				}
				if(document.getElementById('team1clan').checked == false) {	
					if(document.getElementById('team_1').value == \"\") {
						alert('Bitte gib den Namen des Team 1 ein! - Please enter the name for Team 1!');
						document.getElementById('team_1').focus();
						return false;
					}
				}
				if(document.getElementById('team2').value == \"\") {
					alert('Bitte gib den Namen des Team 2 ein! - Please enter the name for Team 2!');
	...
	echo'<form method="post" action="admincenter.php?site=topmatch" onsubmit="return chkFormular()" enctype="multipart/form-data">
...
:
<input type="text" name="minute" value="00" size="2" class="form_off" onFocus="this.className=\'form_on\'" onBlur="this.className=\'form_off\'" /></td>
</tr>
<tr>
...
<td><input type=\"checkbox\" name=\"team1clan\" id=\"team1clan\" value=\"1\" onclick=\"hidd('team1')\" /></td></tr>";
echo'
</table>
</td>
</tr>
...
<td><input type="text" name="team1" id="team_1" size="30" /></td>
</tr>
</table></td>
</tr>
<tr>
...
<td><input type="text" name="team2" id="team2" size="30" /></td>
</tr>
</table></td>
</tr>
<tr>
...
:
<input type="text" name="minute" value="'.$minute.'" size="2" class="form_off" onFocus="this.className=\'form_on\'" onBlur="this.className=\'form_off\'" /></td>
</tr>
<tr>
<td><b>'.$_language->module['matchlink'].'</b></td>
...
<tr>
<td><b>Game</b></td>
<td><select name="game" id="game">'.$games.'</select></td>
</tr>
<tr>
<td><b>'.$_language->module['topmatch_displayed'].'</b></td>
<td>'.$displayed.'</td>
...
<tr>
<td width="130"><b>'.$_language->module['team1'].'</b></td>
<td><input type="text" name="team1" id="team_1" value="'.$ds[team1].'" size="30" /></td>
</tr>
</table></td>
</tr>
<tr>
...
<tr>
<td width="130"><b>'.$_language->module['team2'].'</b></td>
<td><input type="text" name="team2" id="team2" value="'.$ds[team2].'" size="30" /></td>
</tr>
</table></td>
</tr>
<tr>
	...
	if($all) {
		...
		<td align="right"><input type="button" onclick="MM_goToURL(\'parent\',\'admincenter.php?site=topmatch&amp;action=settings\');return document.MM_returnValue" value="Topmatch '.$_language->module['settings'].'" /></td>
			...
			echo'<tr> 
					...
					<td class="'.$td.'" width="15%"><a href="'.$ds['matchlink'].'" target="_blank">'.getinput($ds[league]).'</a></td>
